...
def get_attr_features(soup,hotel_name):
    ...
    try:
        atr_categories = soup.find('span',class_= re.compile('attractionCategories')).text
    except:
        atr_categories=''
    ...
    atr_details = {'Address':strt_address + ',' + ext_address + ',' + locality,
                   'Attraction_Name':hotel_name,                   
                   'about_atrcn':atr_details,
                   'atr_categories':atr_categories,
...
#    print() ['city_name','review_body','nearby_attraction','review_date','review_rating']
...
#    print(start_urls)
    wrt_header = True
    if (os.path.isfile('Attraction_details_' + city + '.csv')):
        wrt_header = False
    ...
    # write in CSV
        if en_type == 'h':
            ...
            with open(filename_rvw, 'a') as f:
                if wrt_header:
                    pd.DataFrame(reviews).to_csv(f, header=['city_name','review_body','nearby_attraction','review_date','review_rating'],\
                            encoding='utf-8')
                else:
            ...
            with open(filename_dtl, 'a') as f:
                if wrt_header:
                    pd.DataFrame(hotel_details_list).to_csv(f, header= \
                        ['Attraction_Name', 'Address','about_atrcn', 'review_keywords','rating', \
                         'num_reviews', 'atr_categories'],encoding='utf-8')
                    wrt_header = False
                else:
            ...
            with open(filename_rvw, 'a') as f:
                if not wrt_header:
                    ...
                    pd.DataFrame(reviews).to_csv(f, header=['city_name','nearby_attraction','review_body','review_date','review_rating'],\
                            encoding='utf-8')
                    wrt_header = False
            with open(filename_dtl, 'a') as f:
                if not wrt_header:
                    ...
                    pd.DataFrame(atr_details_list).to_csv(f, header=['Attraction_Name', 'Address','about_atrcn', 'atr_categories', \
                                'num_reviews','rating','review_keywords'],encoding='utf-8')
                    wrt_header = False
